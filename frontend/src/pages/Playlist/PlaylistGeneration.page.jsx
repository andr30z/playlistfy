...
const PlaylistGeneration = (props) => {
    ...
    debounce((search) => {
      auth
        ...
        .catch((error) => {
          if (is401(error)) {
            throwError(
              "Sua sessão expirou, por favor efetue o login novamente"
            );
          }
        });
      // eslint-disable-next-line
  ...
  const generatePlaylistFromMostListenedSongs = () => {};
    ...
    <div className="playlistContainer wrapper">
      ...
      <div className="searchContainer">
        <div className="explanation-container text-focus-in">
          <p>
            Para um resultado melhor na playlist final, precisamos primeiro de
            uma direção sobre quais tipos de música você quer descobrir.
          </p>
        </div>
        <div className="contentSearch">
          <SearchField search={search.search} setSearch={setSearch} />
          <SearchResults
        ...
        <div className="generatePlaylistButtons">
          <CustomButton
            text="Playlistfy!"
            onSubmit={onSubmit}
            customStyle="button-playlist boxshadow slide-left"
          />
          <CustomButton
            text="Usar músicas preferidas"
            onSubmit={generatePlaylistFromMostListenedSongs}
            customStyle="button-playlist boxshadow slide-left"
          />
        </div>
      </div>
