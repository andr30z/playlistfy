...
routes.get('/oauth', (req, res) => {
    ...
    res.redirect(`https://accounts.spotify.com/authorize?client_id=${CLIENT_ID}&response_type=code&redirect_uri=http://localhost:3000/log&scope=user-read-private%20user-read-email%20user-top-read%20playlist-modify-public`)
});
...
routes.get('/user/data-personalization', (req, res) => {
    ...
    ]).then(Axios.spread((artists, tracks) => {
        if (artists && tracks) {
...
routes.get('/generate-playlist', async (req, res) => {
    ...
    if (!genres) {
        ...
        return recommendedPlaylist.tracks.length === 0 ?
            res.status(404).send('Não foi possível gerar uma playlist com base neste artista :(')
            :
            res.send(recommendedPlaylist);
    }

    if (genres.length === 0) {
        return res.status(404).send('Não foi possível gerar uma playlist com base neste artista :(');
    }

    const seedGenres = filterGenres(genres);
    const artistTopTracks = await getArtist(decTo, artistUrl, { id, country: 'US' })
        .then(resp => resp.data)
        .catch(error => ({
            error,
            status: error.response.status,
...
routes.post('/create-playlist', async (req, res) => {
    ...
    const decTo = decKey(to, null).toString(crypto.enc.Utf8);
    const description = "Playlist feita utilizando o app Playlistfy (www.playlistfy.herokuapp.com). Artista/musica utilizada como base: " + source;
    const data = {
        name: name,
        description: description
    };
    const postRes = await postPlaylist(decTo, data, `https://api.spotify.com/v1/users/${userId}/playlists`)
        .then(resp => ({
            status: resp.status,
...
function decKey(key, ref) {
    return ref ? crypto.AES.decrypt(ref, process.env.CRR) : crypto.AES.decrypt(key, process.env.CRP)
}
